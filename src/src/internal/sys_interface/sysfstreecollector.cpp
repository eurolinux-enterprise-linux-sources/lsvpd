...
/*		devs = getComponentsVector(devs); */
		...
		for (i = (devs.size() - 1); i >= 0; i--) {
			 ...
			 * mAIXName
			 */
			setKernelName( child, child->sysFsNode.getValue());

			if (child->mParent.getValue().length() > 0) {
				/* Setup all child links for discovered devices */
				parent = findComponent(devs, child->mParent.getValue());
				if (parent != NULL) {
					parent->addChild(child->idNode.getValue());
				}
				else {
					cout << "Error: Failed to find parent: '" << child->mParent.getValue()
						<< "' For child device: '" << child->sysFsNode.getValue() << "'" << endl;
				}
			}

			if (child->devBus.getValue() == "scsi")
				scsiGetHTBL(child);  /* Get Host:Target:Bus:Lun data for device */
			...
			const string& newDevDir)
	{
		...
		if (HelperFunctions::file_exists(link)) {
			...
			if (FSWalk::fs_isLink(link)) {
				...
				/* get node_type: /sys/bus/XXX  OR sys/bus/XXX */
				locBeg = 1;
				for (int i = 0; i < 2; i++)
					locBeg = absTargetPath.find("/", locBeg+1);
				locEnd = absTargetPath.find("/", locBeg+1);

				tmp = absTargetPath.substr(locBeg + 1, locEnd - locBeg -1);
				fillMe->devBus.setValue(tmp,
		...
		if (HelperFunctions::file_exists(link)) {
			...
			if (FSWalk::fs_isLink(link)) {
				...
				fillMe->devDriver.setValue(absTargetPath.substr(lastSlash,
											absTargetPath.length() - lastSlash),
											INIT_PREF_LEVEL, __FILE__, __LINE__);
			}
		}

		/* Mash up the /sys/devices link to get some other useful info */
		link = fillMe->sysFsNode.getValue();

		lastSlash = link.rfind("/", link.length()) + 1;
		fillMe->devSysName.setValue(link.substr(lastSlash,
									link.length() - lastSlash),
									INIT_PREF_LEVEL, __FILE__, __LINE__);
		fillMe->devBusAddr.setValue(fillMe->devSysName.getValue(),
									INIT_PREF_LEVEL, __FILE__, __LINE__);

		/* devBusAddr seems to be a very good AIX name -
		 * adding as such, so that if no other, more preferred name is
		 * found, we can use this. */

		fillMe->addAIXName(fillMe->devBusAddr.getValue(), INIT_PREF_LEVEL - 1);
			...
			const string& parentDir, const string& searchDir)
	{
		...
		{
			newDevDir = searchDir + "/" + listing.back();
			listing.pop_back();

			/* If dir entry is a directory, it must be a device */
			if (FSWalk::fs_isDir(newDevDir)) {

				/* if device is shown to NOT be a physical device, do not
				 * add to vector */
				tmp = getAttrValue(newDevDir , "phys_device" );
				if (tmp[0] == '0')
					continue;
				/* Last check - if dir == one of a few known to exist for
				 * each device, this is not a new device */
				tmp = newDevDir;
				int lastSlash = tmp.rfind("/", tmp.length()) + 1;

				tmp = tmp.substr(lastSlash, tmp.length() - lastSlash);
				if ((tmp == "power")
					|| (tmp == "id")
					|| (tmp == "topology")
					|| (tmp == "statistics")
	...
	void SysFSTreeCollector::findDevicePaths(vector<Component*>& devs)
	{
		...
		{
			curPath = "/sys/devices/" + fullList.back();
			fullList.pop_back();

...
//					coutd << "Attempting query, evpd = " << evpd << ", pageCodeInt = " << pageCodeInt <<endl;
	...
	int SysFSTreeCollector::get_mm_scsi(Component *fillMe)
	{
		...
		/* Preferred 'dev' file lookup */
		str = getAttrValue(fillMe->getClassNode(), "dev");

		/* Backup 1: Look in device root dir */
		if (str.length() == 0)
			str = getAttrValue(fillMe->idNode.getValue(), "dev");

		/* Backup 2: Look under generic link */
		if (str.length() == 0) {
			/* Simple lookup failed.  Attempting to follow generic link */
			string link = string (fillMe->sysFsLinkTarget.getValue() + "/generic");
			string genericPath = HelperFunctions::getSymLinkTarget(link);
		...
		if (devClass == "block") fillMe->devAccessMode = S_IFBLK;
...
//		coutd << "Query SCSI dev at: " << fillMe->idNode.getValue() << endl;
		...
		msg = string("vpdupdate: Failed retrieving Major/Minor code for device: ")
			+ fillMe->idNode.getValue();
		logger.log( msg, LOG_WARNING );
